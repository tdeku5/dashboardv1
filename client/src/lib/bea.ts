...
export async function fetchBEANPCESeries(lineNumber: number): Promise<FredObservation[]> {
  const params = new URLSearchParams({ line_number: String(lineNumber) })
  const res = await fetch(`/api/bea/npce?${params}`)
...
export async function fetchBEARPCESeries(lineNumber: number): Promise<FredObservation[]> {
  const params = new URLSearchParams({ line_number: String(lineNumber) })
  const res = await fetch(`/api/bea/rpce?${params}`)
...
export async function fetchBEAGdpContrib(lineNumber: number): Promise<FredObservation[]> {
  const params = new URLSearchParams({ line_number: String(lineNumber) })
  const res = await fetch(`/api/bea/gdp-contrib?${params}`)
