...
// Canada unemployment-rate scenario projection — port of the UK/US mechanical
// model to LFS aggregates (table 14-10-0287, monthly SA). Forward lines are
...
// No proxy badge: the LFS is a true monthly household survey (DIRECT).

const ALL_CODES = [
  'UNRATE_CA',        // unemployment rate, %, SA (rates-side collector)
  'CA_EMPLOYMENT',    // employment level, thousands, SA
  'CA_LABOUR_FORCE',  // labour force level, thousands, SA
] as const
...
export function CALaborProjectionContent() {
  const [allData, setAllData] = useState<Record<string, StatcanPoint[]>>({})
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Canada's population (and hence labour-force) growth is fast — default
  // labour-force growth is 0.15%/month, well above the UK model's 0.05.
  const [growthStr, setGrowthStr] = useState('0.15')
  ...
  useEffect(() => {
    ...
    fetchStatcanBatch(ALL_CODES).then(map => {
      ...
      if (cancelled) return
      setError(e instanceof Error ? e.message : 'Failed to load StatCan LFS data')
      setLoading(false)
    })
    return () => { cancelled = true }
  }, [])

  const uRate = useMemo(() => allData['UNRATE_CA'] ?? [], [allData])
  const latestEmp = useMemo(() => {
    const d = allData['CA_EMPLOYMENT'] ?? []
    return d.length ? d[d.length - 1] : null
  }, [allData])
  const latestLf = useMemo(() => {
    const d = allData['CA_LABOUR_FORCE'] ?? []
    return d.length ? d[d.length - 1] : null
  }, [allData])
  ...
  const projections = useMemo((): number[][] | null => {
    if (!latestEmp || !latestLf) return null
    const emp = latestEmp.value
    const lf = latestLf.value
    return scenarios.map(s => {
      const path: number[] = []
      for (let n = 0; n <= PROJ_MONTHS; n++) {
        const empN = emp + s * n
        const lfN = lf * Math.pow(1 + growth / 100, n)
        path.push(lfN > 0 ? ((lfN - empN) / lfN) * 100 : 0)
      }
      return path
    })
  }, [latestEmp, latestLf, scenarios, growth])
  ...
  if (loading) return <div className={kit.statusBlock}>Loading StatCan LFS series...</div>
      ...
      <div className={kit.section}>
        ...
        <div className={kit.statsRow}>
          <div className={kit.stat}>
            <span className={kit.statLabel}>Employment (CA_EMPLOYMENT)</span>
            <span className={kit.statValue}>{latestEmp ? fmtMillions(latestEmp.value) : '—'}</span>
          </div>
          <div className={kit.stat}>
            <span className={kit.statLabel}>Labour Force (CA_LABOUR_FORCE)</span>
            <span className={kit.statValue}>{latestLf ? fmtMillions(latestLf.value) : '—'}</span>
          </div>
          <div className={kit.stat}>
            <span className={kit.statLabel}>U-Rate (UNRATE_CA)</span>
      ...
      <div className={kit.section}>
        <div className={kit.sectionHeader}>
          <div>
            ...
            <div className={kit.sectionSubtitle}>
              Mechanical scenario arithmetic, not a forecast.
            </div>
          </div>
        </div>
