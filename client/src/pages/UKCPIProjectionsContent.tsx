...
// UK CPI projections — same-month-prior-year MoM pace methodology.
// UK CPI is NSA, so the US carry-forward-recent-MoM model would bake the most
// recent seasonal swing into every future month. Instead, each projected month
// applies the MoM pace observed in the SAME calendar month of prior years:
...
// Both series are DIRECT ONS equivalents — no proxy badges on this page.

const CDIDS = ['D7BT', 'DKC6'] as const
...
function ProjectionSection({ title, cdid, data }: { title: string; cdid: string; data: WD[] }) {
  const model = useMemo(() => {
  ...
  if (data.length === 0) {
    return <div className={kit.statusBlock}>No data for {cdid}</div>
  }
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={model.rows} margin={{ top: 8, right: 16, bottom: 0, left: 8 }}>
            ...
            <ReferenceLine y={2} stroke="rgba(148,163,184,0.6)" strokeWidth={1} strokeDasharray="4 4"
              label={{ value: 'BoE 2%', position: 'insideTopLeft', fill: '#64748B', fontSize: 10 }} />
...
export function UKCPIProjectionsContent() {
  ...
  if (loading) return <div className={kit.statusBlock}>Loading {CDIDS.length} ONS CPI series...</div>
  ...
  return (
    <>
      <div style={{
        ...
        Office for National Statistics (MM23) &mdash; monthly indices, 2015=100, not seasonally adjusted.
